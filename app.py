...
st.markdown(
    ...
    /* =====================================================
       CLEAN MOBILE STACK — explicit mobile rendering
       Desktop wrapper is hidden on mobile; mobile wrapper is
       rendered as one simple vertical stack with one gap token.
       ===================================================== */
    ...
    @media (max-width: 760px) {
        /* Do not collapse the desktop layout anymore: hide it entirely. */
        ...
        /* SINGLE SOURCE OF TRUTH for all mobile section spacing */
        .st-key-mobile_dashboard_wrap > div[data-testid="stVerticalBlock"] {
            gap: 12px !important;
            row-gap: 12px !important;
        }

        /* No individual card margins */
        .st-key-mobile_dashboard_wrap
        > div[data-testid="stVerticalBlock"]
        ...
        .st-key-mobile_monthly_card [data-testid="stPlotlyChart"],
        .st-key-mobile_daily_card [data-testid="stPlotlyChart"],
        .st-key-mobile_platform_card [data-testid="stPlotlyChart"] {
            margin: 0 !important;
            background: transparent !important;
        }

        .st-key-mobile_dashboard_wrap .panel-title {
            font-size: 13px !important;
            margin: 0 0 8px 0 !important;
            color: #17213C !important;
        }

        /* KPI cards are sequential standalone elements now. */
        .st-key-mobile_dashboard_wrap .kpi-card {
            margin: 0 !important;
        }
    }
...
# =========================================================
# EXPLICIT MOBILE DASHBOARD
# =========================================================
# This is rendered separately from the desktop layout so mobile spacing
# no longer depends on collapsed st.columns() wrappers.
with st.container(key="mobile_dashboard_wrap", border=False):
    ...
    with st.container(key="mobile_monthly_card", border=False):
        st.markdown(
            f'<div class="panel-title">MONTHLY SALES TREND '
            f'<span style="font-size:9px;color:#17213C;font-weight:500">'
            f'(Jan {year} – {monthly_title_end})</span></div>',
            unsafe_allow_html=True,
        )
        fig_monthly_mobile = go.Figure(fig_monthly)
        fig_monthly_mobile.update_layout(
            height=230,
            margin=dict(l=8, r=8, t=36, b=55),
            legend=dict(
                orientation="h",
                x=0.5,
                y=1.08,
        ...
        fig_monthly_mobile.update_xaxes(
            tickangle=32,
            tickfont=dict(size=8, color="#59657B"),
        )
        st.plotly_chart(
            fig_monthly_mobile,
            use_container_width=True,
    ...
    # 4. Daily Sales Trend
    with st.container(key="mobile_daily_card", border=False):
        st.markdown(
            f'<div class="panel-title">DAILY SALES TREND '
            f'<span style="font-size:9px;color:#17213C;font-weight:500">'
            f'({selected_ts.strftime("%b %Y")} vs {prev_month_start.strftime("%b %Y")})</span></div>',
            unsafe_allow_html=True,
        )
        st.plotly_chart(
            fig_mobile,
    ...
    # 5. Sales by Platform
    with st.container(key="mobile_platform_card", border=False):
        st.markdown(
            f'<div class="panel-title">SALES BY PLATFORM '
            f'<span style="font-size:9px;color:#17213C;font-weight:500">'
            f'({selected_ts.strftime("%b %Y")} MTD)</span></div>',
            unsafe_allow_html=True,
        )
        st.plotly_chart(
            fig2_mobile,
    ...
    # 6. Top 5 Products
    with st.container(key="mobile_top5_card", border=False):
        st.markdown(
            f'<div class="panel-title">TOP 5 PRODUCTS '
            f'<span style="font-size:9px;color:#17213C;font-weight:500">'
            f'({selected_ts.strftime("%b %Y")} MTD)</span></div>',
            unsafe_allow_html=True,
        )
    ...
    # 7. Quick Insight
    with st.container(key="mobile_insight_card", border=False):
        st.markdown(
            f"""
            <div class="insight-content">
              <div class="insight-title">💡 QUICK INSIGHT</div>
